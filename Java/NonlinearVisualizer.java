...
public class NonlinearVisualizer extends JFrame implements MouseInputListener {
    // region Options
    // region Functions
    float R = 3.1f;
    ...
    private final Function<Float, Float> collatz = (in) -> {
        if (in % 2 == 0)
            ...
            return (float) (in * 3f + 1f) / 2f; // optimized by dividing by two
    };

    private final Function<Float, Float> collatzFractal = (in) -> {
        return 0.5f * in * (float) Math.acos((Math.PI / 2f) * in)
                + (((3f * in) + 1f) / 2f) * (float) Math.asin((Math.PI / 2f) * in);
        // ???
    };
    ...
    private final Function<Float, Float> cosine = (in) -> {
        return (float) Math.cos((float) in / 10f) * 10;
    };

    private final Function<Float, Float> sine = (in) -> {
        return (float) Math.sin((float) in / 10f) * 10;
    };
    ...
    public NonlinearVisualizer(String[] args) {
        getParams(args);
        ...
        for (int i = 0; i < pixels.length; i++) {
            pixels[i] = new Pixel(hasInitVal ? initVal : (i - (pixels.length / 2)));
        }
    ...
    private void getParams(String[] args) {
        for (String arg : args) {
            if (arg.equals("help")) {
                ...
                hasInitVal = true;
                initVal = Float.parseFloat(arg.replace("initVal=", ""));
    ...
    private class DrawPanel extends JPanel { // The class for displaying the visualization of the data
        ...
        public void paintComponent(Graphics g) {
            ...
            if (width < 20 && height < 20) {
                ...
                for (int y = 0; y < height; y++) {
                    for (int x = 0; x < width; x++) {
                        String str = "" + getTripLengthAtCoord(x, y);
