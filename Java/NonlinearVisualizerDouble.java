...
public class NonlinearVisualizerDouble extends JFrame implements MouseInputListener {
    // region Options
    // region Functions
    double R = 3.1;

    private final Function<Double, Double> custom = (in) -> {
        // if (in % 10 == 0)
        // return in % 2;
        // else
        // return in ^ 2;
    ...
    private final Function<Double, Double> collatz = (in) -> {
        if (in % 2 == 0)
            return (in / 2.0);
        else
            return (in * 3.0 + 1.0) / 2.0; // optimized by dividing by two
    };

    private final Function<Double, Double> collatzFractal = (in) -> {
        return 0.5 * in * Math.acos((Math.PI / 2.0) * in)
                + (((3.0 * in) + 1.0) / 2.0) * Math.asin((Math.PI / 2.0) * in);
        // ???
    };

    private final Function<Double, Double> iwanski = (in) -> {
        double n = in;
        double sum = 0.0;
        while (n > 0) {
            double d = n / 10.0;
    ...
    private final Function<Double, Double> rand = (in) -> {
        return Math.random() * 10.0;
    };

    private final Function<Double, Double> cosine = (in) -> {
        return Math.cos(in / 10.0) * 10.0;
    };

    private final Function<Double, Double> sine = (in) -> {
        return Math.sin(in / 10.0) * 10.0;
    };

    private final Function<Double, Double> add = (in) -> {
        return in + 1;
    };

    private final Function<Double, Double> randWalk = (in) -> {
        return in + (int) ((Math.random() - 0.5f) * 10f);
    };
    // endregion
    ...
    private Function<Double, Double> selectedFunction = collatz;
    private String selectedFuncName = "Collatz";

    private Pixel[] pixels; // The values for every pixel

    private double curMax = Double.MIN_VALUE, curMin = Double.MAX_VALUE, curMaxTrip = Double.MIN_VALUE;
    ...
    private double initVal = 0f;

    private boolean shouldRebuildImage = true;

    // Main thread timings
    private ArrayList<Double> timings = new ArrayList<>();
    private double averageTime;

    // Pixel worker thread timings
    private ArrayList<Double> threadTimings = new ArrayList<>();
    private double currentThreadTime;
    private double averageThreadTime;
    // endregion

    public NonlinearVisualizerDouble(String[] args) {
        getParams(args);
        ...
        for (int x = -halfWidth; x <= halfWidth + 1; x++) {
            for (int y = -halfHeight; y <= halfHeight + 1; y++) {
                int index = getIndexAtCoord(x, y);
                pixels[index] = new Pixel(hasInitVal ? initVal : index);
            }
        }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            new NonlinearVisualizerDouble(args);
        });
    }
    ...
    private void getParams(String[] args) {
        for (String arg : args) {
            ...
            } else if (arg.contains("initVal=")) {
                hasInitVal = true;
                initVal = Double.parseDouble(arg.replace("initVal=", ""));
    ...
    private void printHelp() {
        ...
        System.out.println(
                "<initVal=> : The starting value for every pixel. Do not use this parameter if you want each pixel's starting value be their position.");
            ...
            new Thread(new Runnable() {
                public void run() {
                    if (!automatic || maxIterations == 0 || iteration < maxIterations) {
                        ...
                        if (THREAD_COUNT != 1) {
                            int pxSection = (int) Math.floor((float) pixels.length / (float) THREAD_COUNT);
                            ...
                            for (RunThread td : threads) {
                                try {
                                    ...
                                    System.out.println(
                                            "Exception or interruption in Pixel Worker thread: " + e.getStackTrace());
                                }
                            }
                        } else {
                        ...
                        double time = (System.nanoTime() - st) / 1000000.0;
                        if (timings.size() >= 60) {
                            timings.remove(0);
                        }
                        timings.add(time);
    ...
    private class Pixel {
        ...
        protected List<Double> orbit = new ArrayList<>();
        protected int orbitPeriod;
        protected Status status = Status.Incomplete;

        public Pixel(double initVal) {
            set(initVal);
        }
        ...
        public void set(double to) {
            ...
            if (status == Status.Incomplete) {
                if (val == 0) {
                    ...
                    orbit = Arrays.asList(Arrays.copyOfRange(values.toArray(new Double[0]), startInd, values.size()));
    ...
    private class Runner implements Runnable { // I made the runnable a separate class to decouple the runner thread
        ...
        public void run() {
            ...
            double ns = 1000000000 / (double) itersPerSecond;
            long start;
            double delta = 0;
    ...
    private class DrawPanel extends JPanel { // The class for displaying the visualization of the data
        ...
        public void paintComponent(Graphics g) {
            ...
            int xInt = getWidth() / (width + 1);
            int yInt = getHeight() / (height + 1);
            int halfWidth = width / 2;
            int halfHeight = height / 2;
            ...
            if (runningThreads == 0 && shouldRebuildImage) {
                for (int x = -halfWidth; x <= halfWidth + 1; x++) {
                    for (int y = -halfHeight; y <= halfHeight + 1; y++) {
                        ...
                        img.setRGB(Math.min(width - 1, x + halfWidth), Math.min(height - 1, y + halfHeight),
                                col.getRGB());
            ...
            // Draw the position for every pixel (only if it is a small image)
            if (width < 20 && height < 20) {
                g2d.setFont(font);
                FontMetrics fm = g2d.getFontMetrics();
                int h = g2d.getFontMetrics().getHeight();
                for (int y = -halfWidth; y <= halfHeight + 1; y++) {
                    for (int x = -halfHeight; x <= halfWidth + 1; x++) {
                        String str = String.format("(%d, %d)", x, y);
                        int xPos = (x + halfWidth) * xInt + (xInt / 2);
                        int yPos = (y + halfHeight) * yInt + (yInt / 2);
            ...
            // Draw pixel info
            if (pixelPos != null && runningThreads == 0) {
                Pixel px = getPixelAtCoord((int) pixelPos.getX(), (int) pixelPos.getY());
                if (px != null) {
                    str = String.format(
                            "Pixel Info; Pos: (%d, %d) | Value: %f | Status: %s | Orbit: %s | Orbit Period: %d | Values Count: %d | Values: %s",
                            (int) pixelPos.getX() + halfWidth,
                            (int) pixelPos.getY() + halfHeight,
                            px.val, px.status, px.orbit.toString(), px.orbitPeriod, px.values.size(),
                            px.values.toString());
                    ...
                    // Draw selection box
                    g2d.setColor(labelTextColor);
                    int xPos = (int) (pixelPos.getX() * (float) xInt);
                    int yPos = (int) (pixelPos.getY() * (float) yInt);
            ...
            // Draw controls info
            str = "Click once to pause/unpause, double click to switch from manual to automatic iteration.";
            String str2 = "Right click once to toggle coloring based on pixel status, double right cick to toggle coloring based on value or on trip length.";
    ...
    private class RunThread extends Thread { // The thread class for working on pixels
            ...
            synchronized (pixels) {
                ...
                for (int i = start; i < end; i++) {
                    if (pixels[i] == null) {
                        nullCt++;
                        indices.add(i);
                    } else if (pixels[i].status == Status.Incomplete || pixels[i].status == Status.Cycle) {
                        double val = selectedFunction.apply(pixels[i].val);
                        pixels[i].set(val);
                ...
                currentThreadTime += (System.nanoTime() - st) / 1000000.0;

                if (nullCt != 0) {
                    System.out.println(String.format("%d null pixels detected from %d to %d: %s", nullCt, start, end,
        ...
        public void start() {
            if (t == null) {
                t = new Thread(this, "PixelWorker-" + start + "-to-" + end);
                t.start();
            }
        }
    }
    ...
    private int getIndexAtCoord(int x, int y) {
        int halfWidth = width / 2;
        int halfHeight = height / 2;

        return Math.min((width * height) - 1, (((y + halfHeight) * width) + (x + halfWidth)));
    }

    private Pixel getPixelAtCoord(int x, int y) {
        return pixels[getIndexAtCoord(x, y)];
    }
    ...
    private int getTripLengthAtCoord(int x, int y) {
        ...
        if (px != null) {
            return px.orbit.size();
        } else {
            System.out.println("Null pixel at: " + x + ", " + y + ". Index: " + getIndexAtCoord(x, y));
            return 0;
        }
    }

    private double getValueAtCoord(int x, int y) {
        Pixel px = getPixelAtCoord(x, y);
    ...
    private Point2D remapPos(Point2D input) {
        return new Point2D.Double(remap(0, getWidth(), 0, width, (float) input.getX()),
    ...
    private Color remapColor(double min1, double max1, Color a, Color b, float val) {
