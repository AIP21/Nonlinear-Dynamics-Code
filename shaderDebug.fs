        ...
        vec2 f(vec2 z) {
            return cx_add(cx_pow(z, vec2(6, 0)), cx_mul(vec2(-1, 0), cx_pow(z, vec2(3, 0))), vec2(-5, 0)); // z^4 - 1
        }

        // The derivative of the function to visualize
        vec2 fPrime(vec2 z) {
            return cx_add(cx_mul(vec2(6, 0), cx_pow(z, vec2(5, 0))), cx_mul(vec2(-1, 0), vec2(3, 0), cx_pow(z, vec2(2, 0)))); // 4z^3
        }
        ...
        vec2[] roots = vec2[](vec2(1.40799639092612, 0), vec2(-1.21447466605503, 0), vec2(0.607237333027517, -1.05176591305628), vec2(0.607237333027517, 1.05176591305628), vec2(-0.703998195463062, -1.21936064297883), vec2(-0.703998195463062, 1.21936064297883)); // The roots of the function
